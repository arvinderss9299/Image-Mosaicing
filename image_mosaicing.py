...
class ImageMosaicing:
            ...
            for i in range(len(corres_0)):
                if corres_0[i] in inliers_0:
                    cv2.line(full_img, (corres_0[i][1], corres_0[i][0]), (corres_1[i][1] + shift, corres_1[i][0]),
                             (0, 255, 0), thickness=1.5)
                else:
                    cv2.line(full_img, (corres_0[i][1], corres_0[i][0]), (corres_1[i][1] + shift, corres_1[i][0]),
                             (0, 0, 255), thickness=1.5)
        return full_img
    ...
    def disp_img(self, array_of_rgb_images):
        ...
        corner_imgs = []
        i = 0
        for ind in harris_arr:
            while i < len(harris_arr):
                img_to_disp = np.copy(array_of_rgb_images[i])
                for row, response in enumerate(ind):
                    for col, r in enumerate(response):
                        if r > 0:
                            img_to_disp[row, col] = [0, 0, 255]
                corner_imgs.append(img_to_disp)
                i += 1
        return corner_imgs

    def image_stitching(self, img_1, img_2):

        h = np.linalg.inv(self.ransac())
        y, x = img_2.shape[:2]
        p_1 = [[0], [0], [1]]
        p_2 = [[0], [x - 1], [1]]
        p_3 = [[y - 1], [0], [1]]
...
if __name__ == "__main__":
    path_to_images = r"D:\NEU\3 Sem\Computer Vision\Project 2\DanaOffice"
    imgs_arr = []
    imgs_arr2 = []
    ...
    inst = ImageMosaicing([g_1, g_2], "prewitt", (7, 7), 100000, (7, 7), 1000)
